...
var require_dist4 = __commonJS({
  "../node_modules/@supabase/supabase-js/dist/index.cjs"(exports2) {
    var __supabase_functions_js = require_main2();
    var __supabase_postgrest_js = require_dist();
    var __supabase_realtime_js = require_main3();
    var __supabase_storage_js = require_dist3();
    var __supabase_auth_js = require_main4();
    var version = "2.90.0";
    var JS_ENV = "";
  ...
  "../server.js"(exports2) {
    ...
    exports2.login = async (event) => {
      ...
      try {
        ...
        if (error || !user) {
          return response(401, { error: "Credenciales inv\xE1lidas" });
        }
        return response(200, {
          success: true,
          user: {
    ...
    exports2.markTime = async (event) => {
      if (event.httpMethod === "OPTIONS") {
        return response(200, {});
      }
      try {
        const { userId, period, time, date } = JSON.parse(event.body);
        const { data: existing } = await supabase.from("time_records").select("*").eq("user_id", userId).eq("date", date).single();
        let result;
        if (existing) {
          const updateData = {};
          updateData[`${period}_time`] = time;
          result = await supabase.from("time_records").update(updateData).eq("id", existing.id).select();
        } else {
          const insertData = {
            user_id: userId,
            date,
          ...
          result = await supabase.from("time_records").insert(insertData).select();
        }
        if (result.error) {
    ...
    exports2.getTimeRecords = async (event) => {
      if (event.httpMethod === "OPTIONS") {
        return response(200, {});
      }
      try {
    ...
    exports2.markTask = async (event) => {
      if (event.httpMethod === "OPTIONS") {
        return response(200, {});
      }
      try {
        const { userId, taskId, date, completedAt } = JSON.parse(event.body);
        const { data: existing } = await supabase.from("task_completions").select("*").eq("user_id", userId).eq("task_id", taskId).eq("date", date).single();
        let result;
        if (existing) {
          const completions = existing.completions || [];
          completions.push(completedAt);
          result = await supabase.from("task_completions").update({
            completions,
            last_completed_at: completedAt
          }).eq("id", existing.id).select();
        } else {
          result = await supabase.from("task_completions").insert({
            user_id: userId,
            task_id: taskId,
            date,
    ...
    exports2.getTaskCompletions = async (event) => {
      if (event.httpMethod === "OPTIONS") {
        return response(200, {});
      }
      try {
        const { userId, date } = event.queryStringParameters;
        let query = supabase.from("task_completions").select("*, tasks(*)");
        if (userId) {
          query = query.eq("user_id", userId);
        }
        if (date) {
    ...
    exports2.getReport = async (event) => {
      if (event.httpMethod === "OPTIONS") {
        return response(200, {});
      }
      try {
        const { date } = event.queryStringParameters;
        const { data: users } = await supabase.from("users").select("id, username, name").eq("role", "employee");
        const { data: timeRecords } = await supabase.from("time_records").select("*").eq("date", date);
        const { data: taskCompletions } = await supabase.from("task_completions").select("*, tasks(*)").eq("date", date);
...
// ../netlify/functions/get-task-completions.js
var { getTaskCompletions } = require_server();
exports.handler = getTaskCompletions;
//# sourceMappingURL=get-task-completions.js.map
