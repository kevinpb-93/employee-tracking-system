  ...
  try {
    ...
    if (taskName.trim().length === 0) {
      return {
        ...
        body: JSON.stringify({
          success: false,
          error: "El nombre de la tarea no puede estar vac\xEDo"
        })
      };
    }
    ...
    if (!validPeriods.includes(period)) {
      return {
        ...
        body: JSON.stringify({
          success: false,
          error: "Periodo inv\xE1lido. Debe ser: entry, midday o exit"
        })
      };
    }
    ...
    if (!timeRegex.test(deadline)) {
      return {
        ...
        body: JSON.stringify({
          success: false,
          error: "Formato de hora inv\xE1lido. Debe ser HH:MM"
        })
      };
    }
    ...
    const { data: existingTask, error: checkError } = await supabase.from("tasks").select("name").eq("name", taskName.trim());
    if (checkError) {
