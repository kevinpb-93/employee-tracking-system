  ...
  try {
    ...
    if (!["admin", "employee"].includes(role)) {
      return {
        ...
        body: JSON.stringify({
          success: false,
          error: 'Rol inv\xE1lido. Debe ser "admin" o "employee"'
        })
      };
    }
    ...
    const { data: existingUser, error: checkError } = await supabase.from("users").select("username").eq("username", username.trim().toLowerCase());
    if (checkError) {
