...
exports.handler = async (event) => {
  ...
  if (event.httpMethod !== "POST") {
    return {
      statusCode: 405,
      headers,
      body: JSON.stringify({ success: false, error: "M\xE9todo no permitido" })
    };
  }
  try {
    ...
    if (newName.trim().length === 0) {
      return {
        ...
        body: JSON.stringify({
          success: false,
          error: "El nombre no puede estar vac\xEDo"
        })
      };
    }
    ...
    if (!user || user.role !== "admin") {
      return {
        ...
        body: JSON.stringify({
          success: false,
          error: "No tienes permisos para realizar esta acci\xF3n"
        })
      };
    }
    const { data: tasks, error: taskCheckError } = await supabase.from("tasks").select("id, name").eq("id", taskId);
    console.log("B\xFAsqueda de tarea:", { tasks, taskCheckError });
    if (taskCheckError) {
    ...
    const { error: updateError } = await supabase.from("tasks").update({ name: newName.trim() }).eq("id", taskId);
    console.log("Resultado update:", { updateError });
    if (updateError) {
      console.error("Error actualizando:", updateError);
      throw updateError;
    }
    const { data: verifyTask, error: verifyError } = await supabase.from("tasks").select("id, name").eq("id", taskId).single();
    console.log("Verificaci\xF3n post-update:", { verifyTask, verifyError });
    if (verifyError || !verifyTask) {
      throw new Error("No se pudo verificar la actualizaci\xF3n");
    }
    if (verifyTask.name !== newName.trim()) {
      throw new Error("La tarea no se actualiz\xF3 correctamente");
    }
    return {
      statusCode: 200,
      headers,
