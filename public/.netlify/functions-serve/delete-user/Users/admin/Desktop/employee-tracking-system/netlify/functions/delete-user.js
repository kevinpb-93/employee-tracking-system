...
exports.handler = async (event) => {
  ...
  try {
    ...
    console.log("Solicitud de eliminaci\xF3n:", { userId, adminId });
    if (!userId || !adminId) {
      return {
        statusCode: 400,
        headers,
    ...
    const { data: admin, error: adminError } = await supabase.from("users").select("role").eq("id", adminId).maybeSingle();
    ...
    const { data: userToDelete, error: userCheckError } = await supabase.from("users").select("id, name, username, role").eq("id", userId).maybeSingle();
    ...
    const { error: deleteTimeRecordsError } = await supabase.from("time_records").delete().eq("user_id", userId);
    if (deleteTimeRecordsError) {
      console.error("Error eliminando time_records:", deleteTimeRecordsError);
    }
    const { error: deleteTaskCompletionsError } = await supabase.from("task_completions").delete().eq("user_id", userId);
