...
exports.handler = async (event, context) => {
  console.log("\u{1F5D1}\uFE0F Iniciando limpieza autom\xE1tica de datos antiguos...");
  try {
    const sevenDaysAgo = /* @__PURE__ */ new Date();
    sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
    const cutoffDate = sevenDaysAgo.toISOString().split("T")[0];
    console.log(`\u{1F4C5} Eliminando registros anteriores a: ${cutoffDate}`);
    const { data: deletedTimeRecords, error: timeError } = await supabase.from("time_records").delete().lt("date", cutoffDate);
    if (timeError) {
      console.error("\u274C Error eliminando time_records:", timeError);
    } else {
      console.log("\u2705 Registros de horarios eliminados");
    }
    const { data: deletedTaskCompletions, error: taskError } = await supabase.from("task_completions").delete().lt("date", cutoffDate);
    if (taskError) {
      console.error("\u274C Error eliminando task_completions:", taskError);
    } else {
      console.log("\u2705 Completaciones de tareas eliminadas");
    }
    const { count: timeCount } = await supabase.from("time_records").select("*", { count: "exact", head: true });
    const { count: taskCount } = await supabase.from("task_completions").select("*", { count: "exact", head: true });
    const summary = {
      success: true,
    ...
    console.log("\u{1F389} Limpieza completada:", summary);
    return {
      statusCode: 200,
      body: JSON.stringify(summary)
    };
  } catch (error) {
    console.error("\u{1F4A5} Error durante la limpieza:", error);
    return {
      statusCode: 500,
      body: JSON.stringify({
        success: false,
        error: error.message,
        message: "Error durante la limpieza autom\xE1tica"
      })
    };
  }
};
//# sourceMappingURL=scheduled-cleanup.js.map
