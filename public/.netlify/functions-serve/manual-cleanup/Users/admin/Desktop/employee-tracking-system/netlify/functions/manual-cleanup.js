...
exports.handler = async (event, context) => {
  ...
  if (event.httpMethod !== "POST") {
    return response(405, { error: "M\xE9todo no permitido" });
  }
  try {
    ...
    const { count: timeCountBefore } = await supabase.from("time_records").select("*", { count: "exact", head: true }).lt("date", dateString);
    const { count: taskCountBefore } = await supabase.from("task_completions").select("*", { count: "exact", head: true }).lt("date", dateString);
    await supabase.from("time_records").delete().lt("date", dateString);
    await supabase.from("task_completions").delete().lt("date", dateString);
    const { count: timeCountAfter } = await supabase.from("time_records").select("*", { count: "exact", head: true });
    const { count: taskCountAfter } = await supabase.from("task_completions").select("*", { count: "exact", head: true });
    return response(200, {
      success: true,
...
//# sourceMappingURL=manual-cleanup.js.map
